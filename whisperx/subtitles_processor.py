...
from whisperx.conjunctions import get_comma, get_conjunctions
            ...
            if advanced_splitting:
                ...
                for wi, word in enumerate(words):
                    if "start" not in word or "end" not in word:
                        self.estimate_timestamp_for_word(words, wi, next_segment_start_time)
    ...
    def generate_subtitles_from_split_points(self, segment, split_points, next_start_time=None):
        ...
        for split_point in split_points:
            ...
            if isinstance(fragment_words[0], dict):
                ...
                text = prefix.join(word["word"] for word in fragment_words)
            else:
        ...
        with open(filename, "w", encoding="utf-8") as file:
            ...
            if advanced_splitting:
                for idx, subtitle in enumerate(subtitles, 1):
                    start_time = format_timestamp(float(subtitle["start"]), self.is_vtt)
                    end_time = format_timestamp(float(subtitle["end"]), self.is_vtt)
