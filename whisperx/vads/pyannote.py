        ...
        for t, y in zip(timestamps[1:], scores[1:], strict=False):
            if is_active:
                ...
                if curr_duration > self.max_duration:
                    ...
                    min_idx = search_after + int(np.argmin(curr_scores[search_after:]))
                    min_score_t = curr_timestamps[min_idx]
                    segments.append(
                        _SpeechSegment(start - self.pad_onset, min_score_t + self.pad_offset)
                    )
...
class Pyannote(Vad):
    def __init__(self, device=None, token=None, model_fp=None, **kwargs):  # noqa: ARG002
        ...
        # Lazy import: segmentation weights load only when VAD is used.
    ...
    def __call__(self, audio, **kwargs):  # noqa: ARG002 - Vad interface conformance
        ...
        return [SegmentX(float(s.start), float(s.end), "UNKNOWN") for s in speech_segments]
